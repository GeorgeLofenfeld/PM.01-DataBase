...
wchar_t* Layout::get_available_keys()
{
	...
	*/
	static wchar_t keys[MAX_AVAILABLE_KEYS_COUNT * MAX_LAYOUT_OBJECTS_COUNT]; // массив для доступных клавиш клавиатуры
	...
	if (chosen_index != -1) { // chose_index - индекс выбранного объекта
		wcsncat(keys, objects[chosen_index]->get_available_keys(), MAX_AVAILABLE_KEYS_COUNT); // добавили доступные клавиши для выбранного layout_object
	}
	for (int i = 0; i < object_cnt; i++) // итерируемся по всем объектам данного layout'a
		if (objects[i]->get_is_background() && i != chosen_index) // get_is_background - метод класса layout_object, может ли данный объект реагировать на клавиши не будучи выбранным
			wcsncat(keys, objects[i]->get_available_keys(), MAX_AVAILABLE_KEYS_COUNT); // добавили доступные клавиши для выбранного layout_object
